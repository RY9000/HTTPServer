...
const char *const ROOT = "/HTTPServer/root/"; // path to root, like "/home/HTTPServer/root/", the last / is needed
...
int main()
{
	...
	sqlPool _sqlPool(NUM_THREADS);
	if (!_sqlPool.initSqlPool(HOST, SQL_USER, SQL_PASSWORD, DATABASE, SQL_PORT))
	{
	...
	for (int i = 0; i < MAX_CONNECTIONS; ++i)
	{
		conns.emplace_back(std::make_unique<connection>(REQ_BUF_SIZE, RESP_HEAD_BUF_SIZE, RESP_BODY_BUF_SIZE, ROOT, homePageAddr,
			homePageSize, cssAddr, cssSize, jsAddr, jsSize, epollFd, i, _sqlPool, _sessions));
	}

	threadPool _threadPool(NUM_THREADS, evtQue, conns, MAX_CONNECTIONS, TIMEOUT, epollFd, _sessions);
	...
	while (!stopRunning)
	{
		...
		for (int i = 0; i < numEvents; ++i)
		{
			...
			{
				evtQue.push_back({ listenFd, EventType::ACCEPT });
			}
			else if (events[i].data.fd == timerFd)
			{
				evtQue.push_back({ (int)events[i].data.fd, EventType::ACTIVITY_TIMER });
			}
			...
			{
				evtQue.push_back({ (int)events[i].data.fd, EventType::RESET });
			}
			else if (events[i].events & EPOLLIN)
			{
				evtQue.push_back({ (int)events[i].data.fd, EventType::IN });
			}
			else if (events[i].events & EPOLLOUT)
			{
				evtQue.push_back({ (int)events[i].data.fd, EventType::OUT });
			}
		}
	}
